...
from cache_key_audit import __version__
from cache_key_audit.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from cache_key_audit.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='cache-key-audit',
        description='Find tenant and user isolation risks in cache key designs.',
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='cache key design notes or config text',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"cache-key-audit: error: {exc}", file=sys.stderr)
        return 1
